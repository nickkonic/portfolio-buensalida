...
import { useUpdateAboutProfile, AboutProfile, AboutProfilePayload } from "@/hooks/use-about";
...
export function EditProfileForm({ profile, onBack, onSuccess }: EditProfileFormProps) {
  const updateProfileMutation = useUpdateAboutProfile(profile.id);
  ...
  useEffect(() => {
    if (profile) {
      ...
      setScheduleCallUrl(profile.scheduleCallUrl || "");
      ...
      if (profile.technicalSkills) {
        setTechnicalSkills(
          profile.technicalSkills.map((sk) => ({
            ...
            url: sk.url || "",
            link: sk.link || "",
            category: sk.category || "Programming Languages",
            order: sk.order || 0,
          }))
        );
      }
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] overflow-hidden bg-background">
      ...
      <div className="flex items-center justify-between p-4 border-b border-border bg-background/95 backdrop-blur shrink-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-lg leading-tight">Edit Profile</h1>
            <p className="text-[10px] text-muted-foreground hidden md:block">Updating your live presence</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            {updateProfileMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <FileText className="h-4 w-4 mr-2" />}
            Save Changes
          </Button>
        </div>
      </div>
